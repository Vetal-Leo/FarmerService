...
namespace Servises.Growing
{
    public partial class GrowingService
    {
        ...
        #region SilageComings

        public List<GrowingFieldComings> GetSilageComings(int userId)
        {
            return Calculate(_growingFieldComingsRepository.Get().Where(i => i.User.Id == userId && i.GrowingType.Name == "Silage").ToList());
        }
        ...
        private static DateTime? GetExpirationDateForSilageCulture(DateTime landingdate, string culturetype)
        {
            switch (culturetype)
            {
                case "Кукуруза":
                ...
                case "Подсолнечник":
                    return landingdate.AddDays(52);
                case "Кормовая капуста":
                    return landingdate.AddDays(140);
                case "Топинамбур":
                    return landingdate.AddDays(125);
                case "Горчица белая":
                    return landingdate.AddDays(62);
                case "Рапс озимый":
                    return landingdate.AddDays(305);
                case "Сорго":
        ...
        public GrowingFieldComings GetSilageComingById(int id)
        {
            ...
            return new GrowingFieldComings
            {
                ...
                GrowingCultureses = _growingCulturesRepository.Get().Where(c => c.GrowingType.Name == "Silage")
            };
        }

        public async Task AddNewSilageById(int id, FormCollection collection)
        {
            var picked = _growingFieldComingsRepository.Get().Where(i => i.Id == id).ToList()[0];
        ...
        public void DeleteSilageById(int id)
        {
        ...
        public List<GrowingCharges> GetSilageCharges(int userId)
        {
            return Calculate(_growingChargesRepository.Get().Where(i => i.User.Id == userId && i.GrowingType.Name == "Silage").ToList());
        }


        public List<string> GetSilageNamesById(int id)
        {
            var grovings = _growingFieldComingsRepository.Get().Where(i => i.User.Id == id && i.GrowingType.Name == "Silage").ToList();
            return grovings.Select(item => item.Name).ToList();
        }

        public async Task AddNewSilageCharges(int id, FormCollection collection)
        {
        ...
        public GrowingCharges GetSilageChargesById(int id)
        {
        ...
        public async Task AddNewSilageChargesById(int id, FormCollection collection)
        {
        ...
        public void DeleteSilageChargesById(int id)
        {
        ...
        public List<GrowingFieldProfits> GetSilageProfits(int userId)
        {
            return Calculate(_growingFieldProfitsRepository.Get().Where(i => i.User.Id == userId && i.GrowingType.Name == "Silage").ToList(), userId);
        }

        public GrowingFieldProfits GetSilageProfitById(int id)
        {
        ...
        public async Task AddNewSilageProfitById(int id, FormCollection collection)
        {
            var picked = _growingFieldProfitsRepository.Get().Where(i => i.Id == id).ToList()[0];
